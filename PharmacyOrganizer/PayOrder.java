...
public class PayOrder implements Option {
	...
	public void oper(Database database, JFrame parent, Employee e) {
		...
		frame.addToGrid(new JLabel("Paid:", 18));
		JTextField paidAmount = new JTextField(18);
		frame.addToGrid(paidAmount);
		...
		JButton pay = new JButton("Pay", 18);
			...
			public void mouseClicked(MouseEvent event) {
				try {
					if (paidAmount.getText().equals("")) {
						JOptionPane.showMessageDialog(frame, "Enter paid amount!");
						return;
					}
					try {
						Double.parseDouble(paidAmount.getText());
					} catch (Exception e4) {
						JOptionPane.showMessageDialog(frame, "Enter valid paid amount!");
						return;
					}
					...
					if (paid<total) {
						JOptionPane.showMessageDialog(frame, "Paid is less than total!");
						return;
					} else {
						...
						String update = "UPDATE `orders` SET `Cashier`='"+e.getID()+
								"',`Paid`='"+paid+"',`Change_`='"+change+
								"',`PayDateTime`='"+payDateTime+"' WHERE `ID` = "
								+ID.getSelectedItem().toString()+" ;";
						database.getStatement().execute(update);
						JOptionPane.showMessageDialog(frame, "Change: "+change);
						JOptionPane.showMessageDialog(frame, "Order paid successfully");
						frame.dispose();
					}
