	...
	public void oper(Database database, JFrame parent, Employee e) {
		...
		confirm.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (o.getPaid()>0) {
					JOptionPane.showMessageDialog(frame, "Refund: "+o.getTotal());
				}
				try {
					String delete1 = "DELETE FROM `orders` WHERE `ID` = "+
								ID.getSelectedItem()+" ;";
					String delete2 = "DELETE FROM `ordersitems` WHERE `Order` = "+
								ID.getSelectedItem()+" ;";
					database.getStatement().execute(delete1);
					database.getStatement().execute(delete2);
